...
rm -f .integration-test-cache/coverage/coverage.*
COLLECT_COVERAGE=TRUE poetry run pytest tests/unit tests/integration --html-report="${TESTREPORT_BUILDINFO}" --title="Jupiter Tests"
...
mv .integration-test-cache/coverage-html ${COVERAGE_BUILDINFO}
