...
from models.basic import EntityId, ADate, Timestamp
from models.framework import AggregateRoot, Event, BAD_REF_ID
...
class MetricEntry(AggregateRoot):
    ...
    def new_metric_entry(
            metric_ref_id: EntityId, collection_time: ADate, value: float, notes: Optional[str],
        ...
        metric_entry = MetricEntry(
            ...
            _value=value,
            _notes=notes)
        metric_entry.record_event(MetricEntry.Created(timestamp=created_time))
        return metric_entry
    ...
    def change_collection_time(self, collection_time: ADate, modification_time: Timestamp) -> 'MetricEntry':
        ...
        self.record_event(MetricEntry.Updated(timestamp=modification_time))
        return self
