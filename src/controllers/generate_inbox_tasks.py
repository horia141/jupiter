    ...
    def recurring_tasks_gen(
        ...
        if SyncTarget.PROJECTS in gen_targets:
            for project in self._projects_service.load_all_projects(filter_keys=filter_project_keys):
                ...
                if len(all_recurring_tasks) == 0:
                    continue
                all_collection_inbox_tasks = self._inbox_tasks_service.load_all_inbox_tasks(
        ...
        if SyncTarget.METRICS in gen_targets:
            with self._metric_engine.get_unit_of_work() as metric_uow:
                all_metrics = metric_uow.metric_repository.find_all(filter_keys=filter_metric_keys)
        ...
        if SyncTarget.PRM in gen_targets:
            with self._prm_engine.get_unit_of_work() as prm_uow:
                all_persons = prm_uow.person_repository.find_all(filter_ref_ids=filter_person_ref_ids)

                all_catch_up_inbox_tasks = self._inbox_tasks_service.load_all_inbox_tasks(
                    allow_archived=True,
                ...
                for person in all_persons:
                    ...
                    # MyPy not smart enough to infer that if (not A and not B) then (A or B)
                    project = self._projects_service.load_project_by_ref_id(person.catch_up_params.project_ref_id)
    ...
    def _generate_inbox_tasks_for_person(
        ...
        if found_task:
            ...
            self._inbox_tasks_service.create_inbox_task_for_person(
                ...
                recurring_task_timeline=schedule.timeline,
                recurring_task_period=catch_up_params.period,
                recurring_task_gen_right_now=right_now,
                eisen=catch_up_params.eisen,
                difficulty=catch_up_params.difficulty,
                actionable_date=schedule.actionable_date,
