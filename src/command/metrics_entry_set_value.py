"""Command for setting the value of a metric entry."""
...
class MetricsEntrySetValue(command.Command):
    """Command for setting the value of a metric entry."""
    ...
    def name() -> str:
        ...
        return "metrics-entry-set-value"

    @staticmethod
    def description() -> str:
        """The description of the command."""
        return "Change the value of a metric entry"

    def build_parser(self, parser: ArgumentParser) -> None:
        """Construct a argparse parser for the command."""
        parser.add_argument("--id", dest="ref_id", required=True, help="The id of the metric entry")
        parser.add_argument("--value", dest="value", required=True, type=float, help="The value of the metric entry")

    def run(self, args: Namespace) -> None:
        """Callback to execute when the command is invoked."""
        ref_id = self._basic_validator.entity_id_validate_and_clean(args.ref_id)
        value = args.value
        self._metrics_controller.set_metric_entry_value(ref_id, value)
