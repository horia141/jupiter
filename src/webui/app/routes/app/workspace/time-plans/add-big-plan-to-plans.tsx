...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    switch (form.intent) {
      case "add": {
        await apiClient.timePlans.timePlanAssociateBigPlanWithPlan({
          big_plan_ref_id: query.bigPlanRefId,
          time_plan_ref_ids: form.targetTimePlanRefIds,
          kind: form.kind,
          feasability: form.feasability,
        });

        return redirect(`/app/workspace/big-plans/${query.bigPlanRefId}`);
      }
...
export default function AddBigPlanToPlans() {
...
export const ErrorBoundary = makeLeafErrorBoundary(
  (params, searchParams) =>
    `/app/workspace/big-plans/${searchParams.get("bigPlanRefId")}`,
  ParamsSchema,
  {
    error: () =>
      `There was an error adding the big plan to time plans! Please try again!`,
  },
);
