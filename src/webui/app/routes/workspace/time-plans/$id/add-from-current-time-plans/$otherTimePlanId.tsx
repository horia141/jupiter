...
export const shouldRevalidate: ShouldRevalidateFunction =
  ...
  export async function action({ request, params }: ActionArgs) {
      ...
      try {
          switch (form.intent) {
              case "add": {
                  await getLoggedInApiClient(session).timePlans.timePlanAssociateWithActivities({
                      ref_id: id,
...
export default function TimePlanAddFromCurrentTimePlans() {
    ...
    const [targetActivitiesRefIds, setTargetActivitiesRefIds] = useState<Set<string>>(new Set(
        loaderData.otherActivities
          .filter(s => loaderData.mainActivities.findIndex(r => r.target === s.target && r.target_ref_id === s.target_ref_id) !== -1)
          .map(tpa => tpa.ref_id)
    ));

    const sortedOtherActivities = sortTimePlanActivitiesNaturally(loaderData.otherActivities);
    ...
    return (
        <LeafPanel
            ...
            <Card sx={{ marginBottom: "1rem" }}>
                <GlobalError actionmainResult={actionData} />
                <CardContent>
                    {sortedOtherActivities.map(activity => (
                      <TimePlanActivityCard
                        ...
                        onClick={a => setTargetActivitiesRefIds(at => toggleActivitiesRefIds(at, activity.ref_id))}
                        />
                    ))}
                </CardContent>
              ...
              <TimePlanCard
                ...
                <input name="targetActivitiesRefIds" type="hidden" value={targetActivitiesRefIds.join(",")} />
...
export const CatchBoundary = makeCatchBoundary(
    () =>
      `Could not find time plan  #${useParams().id}:#${useParams().otherTimePlanId}`
  );
  
  export const ErrorBoundary = makeErrorBoundary(
    () =>
      `There was an error loading time plan activity #${useParams().id}:#${useParams().otherTimePlanId}. Please try again!`
  );

  function toggleActivitiesRefIds(ActivitiesRefIds: Set<string>, newRefId: string): Set<string> {
