...
from jupiter.core.use_cases.time_plans.load import TimePlanLoadResult, TimePlanLoadUseCase
from rich.console import Console
from rich.text import Text
from rich.tree import Tree

from jupiter.cli.command.rendering import big_plan_status_to_rich_text, difficulty_to_rich_text, eisen_to_rich_text, entity_id_to_rich_text, entity_name_to_rich_text, inbox_task_status_to_rich_text, period_to_rich_text, source_to_rich_text, time_plan_activity_feasability_to_rich_text, time_plan_activity_kind_to_rich_text, time_plan_source_to_rich_text
    ...
    def _render_result(
        ...
        for activity in sorted_activities:
            if activity.target == TimePlanActivityTarget.INBOX_TASK:
                ...
                target_status_text = inbox_task_status_to_rich_text(target_inbox_task.status, target_inbox_task.archived)
            else:
                target_big_plan = big_plans_by_ref_id[activity.target_ref_id]
                target_name_text = entity_name_to_rich_text(target_big_plan.name)
                target_status_text = big_plan_status_to_rich_text(target_big_plan.status, target_big_plan.archived)

            activity_text = Text("")\
                .append(entity_id_to_rich_text(activity.ref_id))\
                .append(" Work on ")\
                .append("inbox task " if activity.target == TimePlanActivityTarget.INBOX_TASK else "big plan ")\
                ...
                .append(" ")\
                .append(time_plan_activity_feasability_to_rich_text(activity.feasability))
            
            activity_tree.add(activity_text)

        if len(result.completed_nontarget_inbox_tasks) > 0:
            completed_nontarget_tree = rich_tree.add("Completed Non-targets Inbox Tasks")
            ...
            for inbox_task in sorted_inbox_tasks:
                ...
                inbox_task_text.append(" ")\
                    .append(entity_id_to_rich_text(inbox_task.ref_id))\
                    .append(f" {inbox_task.name}")\
                    .append(" ")\
                    .append(source_to_rich_text(inbox_task.source))\
                    .append(" ")\
                    .append(eisen_to_rich_text(inbox_task.eisen))
        ...
        if result.completed_nottarget_big_plans and len(result.completed_nottarget_big_plans) > 0:
            ...
            for big_plan in sorted_big_plans:
                ...
                big_plan_text.append(" ")\
                    .append(entity_id_to_rich_text(big_plan.ref_id))\
