...
class SqliteBigPlanCollectionRepository(BigPlanCollectionRepository):
    ...
    def __init__(self, connection: AsyncConnection, metadata: MetaData) -> None:
        ...
        self._big_plan_collection_event_table = build_event_table(
            self._big_plan_collection_table,
            metadata,
        )

    async def create(self, entity: BigPlanCollection) -> BigPlanCollection:
        ...
        await upsert_events(
            self._connection,
            self._big_plan_collection_event_table,
            entity,
        )
        return entity

    async def save(self, entity: BigPlanCollection) -> BigPlanCollection:
    ...
    async def load_by_parent(self, parent_ref_id: EntityId) -> BigPlanCollection:
        """Load a big plan collection for a given project."""
        query_stmt = select(self._big_plan_collection_table).where(
            self._big_plan_collection_table.c.workspace_ref_id
...
class SqliteBigPlanRepository(BigPlanRepository):
    ...
    async def create(self, entity: BigPlan) -> BigPlan:
        """Create the big plan."""
        ref_id_kw = {}
        if entity.ref_id != BAD_REF_ID:
            ref_id_kw["ref_id"] = entity.ref_id.as_int()
        ...
        entity = entity.assign_ref_id(EntityId(str(result.inserted_primary_key[0])))
        await upsert_events(self._connection, self._big_plan_event_table, entity)
        return entity

    async def save(self, entity: BigPlan) -> BigPlan:
        """Save the big plan."""
        result = await self._connection.execute(
    ...
    async def find_all_generic(
        ...
        **kwargs: EntityLinkFilterCompiled,
        ...
        query_stmt = compile_query_relative_to(query_stmt, self._big_plan_table, kwargs)

        results = await self._connection.execute(query_stmt)
        return [self._row_to_entity(row) for row in results]

    async def remove(self, ref_id: EntityId) -> BigPlan:
        """Remove a bit plan."""
        query_stmt = select(self._big_plan_table).where(
            self._big_plan_table.c.ref_id == ref_id.as_int(),
        )
        ...
        await remove_events(self._connection, self._big_plan_event_table, ref_id)
        return self._row_to_entity(result)
