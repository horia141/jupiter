...
class SqliteBigPlanRepository(
    SqliteLeafEntityRepository[BigPlan], BigPlanRepository
):
    """The big plan repository."""

    async def find_completed_in_range(self, parent_ref_id: EntityId, allow_archived: bool, filter_start_completed_date: ADate, filter_end_completed_date: ADate, filter_exclude_ref_ids: Iterable[EntityId] | None = None) -> list[BigPlan]:
        ...
        query_stmt = query_stmt.where(
            self._table.c.status.in_(s.value for s in BigPlanStatus.all_completed_statuses())
        ).where(
