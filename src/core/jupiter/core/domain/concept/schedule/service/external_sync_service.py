...
class ScheduleExternalSync:
    ...
    async def do_it(
        ...
        async with self._domain_storage_engine.get_unit_of_work() as uow:
            ...
            sync_logs = await uow.get_for(ScheduleSyncLog).find_all(
            ...
            sync_log_entry = ScheduleSyncLogEntry.new_log_entry(
                ctx,
                schedule_sync_log_ref_id=sync_log.ref_id,
            )
            sync_log_entry = await uow.get_for(ScheduleSyncLogEntry).create(
                sync_log_entry
            )
        ...
        async with self._domain_storage_engine.get_unit_of_work() as uow:
            ...
            sync_log_entry = await uow.get_for(ScheduleSyncLogEntry).save(
                sync_log_entry
            )
    ...
    async def _process_schedule_stream(
        ...
        sync_log_entry: ScheduleSyncLogEntry,
    ) -> ScheduleSyncLogEntry:
