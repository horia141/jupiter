...
class SmallScreenHomeTabWidgetPlacement(HomeTabWidgetPlacement):
    ...
    _ALLOWED_DIMENSIONS: ClassVar[list[WidgetDimension]] = [
    ...
    def add_widget(
        ...
        if (
            geometry.dimension
            not in SmallScreenHomeTabWidgetPlacement._ALLOWED_DIMENSIONS
        ):
        ...
        if geometry.row < len(self.sections):
            ...
            if geometry.dimension == WidgetDimension.DIM_kx1:
                raise InputValidationError(
                    "kx1 dimension can only be used as the last element"
                )
            ...
            return SmallScreenHomeTabWidgetPlacement(
                kind=self.kind, sections=new_sections
            )
