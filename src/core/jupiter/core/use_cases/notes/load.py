...
class NoteLoadUseCase(AppTransactionalLoggedInReadOnlyUseCase[NoteLoadArgs, NoteLoadResult]):
    ...
    async def _perform_transactional_read(
        ...
        subnotes = await uow.note_repository.find_all_with_filters(
           note.note_collection_ref_id, allow_archived=args.allow_archived, filter_parent_note_ref_ids=[note.ref_id]
        )
