...
from sqlalchemy import insert, MetaData, Table, Column, DateTime, String, Integer, ForeignKey, JSON
...
class SqliteMutationUseCaseInvocationRecordRepository(MutationUseCaseInvocationRecordRepository):
    ...
    def __init__(self, connection: Connection, metadata: MetaData) -> None:
        ...
        self._mutation_use_case_invocation_record_table = Table(
            ...
            metadata,
            Column('owner_ref_id', Integer, ForeignKey('workspace.ref_id'), primary_key=True),
    ...
    def create(self, invocation_record: MutationUseCaseInvocationRecord[UseCaseArgs]) -> None:
        ...
        self._connection.execute(
            insert(self._mutation_use_case_invocation_record_table)
            .values(
