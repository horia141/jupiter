...
from jupiter.domain.push_integrations.slack.infra.slack_task_collection_repository \
    import SlackTaskCollectionRepository, SlackTaskCollectionAlreadyExistsError, SlackTaskCollectionNotFoundError
...
class SqliteSlackTaskCollectionRepository(SlackTaskCollectionRepository):
    ...
    def __init__(self, connection: Connection, metadata: MetaData) -> None:
        ...
        self._slack_task_collection_table = Table(
            ...
            Column(
                'push_integration_group_ref_id', Integer, ForeignKey("push_integration_group_ref_id.ref_id"),
                unique=True, index=True, nullable=False),
            Column('generation_project_ref_id', Integer, ForeignKey("project.ref_id"), nullable=False),
            keep_existing=True)
        self._slack_task_collection_event_table = build_event_table(self._slack_task_collection_table, metadata)
    ...
    def save(self, entity: SlackTaskCollection) -> SlackTaskCollection:
        ...
        if result.rowcount == 0:
            raise SlackTaskCollectionNotFoundError("The slack task collection does not exist")
        upsert_events(self._connection, self._slack_task_collection_event_table, entity)
        return entity

    def load_by_id(self, ref_id: EntityId, allow_archived: bool = False) -> SlackTaskCollection:
        """Retrieve a slack task collection."""
        query_stmt = \
            select(self._slack_task_collection_table)\
            .where(self._slack_task_collection_table.c.ref_id == ref_id.as_int())
        if not allow_archived:
            query_stmt = query_stmt.where(self._slack_task_collection_table.c.archived.is_(False))
    ...
    def load_by_parent(self, parent_ref_id: EntityId) -> SlackTaskCollection:
        ...
        query_stmt = \
            select(self._slack_task_collection_table)\
            .where(self._slack_task_collection_table.c.push_integration_group_ref_id == parent_ref_id.as_int())
        result = self._connection.execute(query_stmt).first()
        if result is None:
            raise SlackTaskCollectionNotFoundError(
    ...
    def _row_to_entity(row: Result) -> SlackTaskCollection:
        return SlackTaskCollection(
            ...
            events=[],
            push_integration_group_ref_id=EntityId.from_raw(str(row["push_integration_group_ref_id"])),
            generation_project_ref_id=EntityId.from_raw(str(row["generation_project_ref_id"])))
...
class SqliteSlackTaskRepository(SlackTaskRepository):
    ...
    def __init__(self, connection: Connection, metadata: MetaData) -> None:
        ...
        self._slack_task_table = Table(
            ...
            Column('last_modified_time', DateTime, nullable=False),
            Column('archived_time', DateTime, nullable=True),
            Column('slack_task_collection_ref_id', Integer, ForeignKey("slack_task_collection.ref_id"), nullable=False),
            ...
            keep_existing=True)
        self._slack_task_event_table = build_event_table(self._slack_task_table, metadata)
    ...
    def save(self, entity: SlackTask) -> SlackTask:
        ...
        result = self._connection.execute(
            ...
            .values(
                ...
                has_generated_task=entity.has_generated_task))
        if result.rowcount == 0:
            raise SlackTaskNotFoundError(f"Slack task with id {entity.ref_id} does not exist")
        upsert_events(self._connection, self._slack_task_event_table, entity)
        return entity

    def load_by_id(self, ref_id: EntityId, allow_archived: bool = False) -> SlackTask:
        """Retrieve the slack task."""
        query_stmt = select(self._slack_task_table).where(self._slack_task_table.c.ref_id == ref_id.as_int())
        if not allow_archived:
    ...
    def find_all(
            ...
            filter_ref_ids: Optional[Iterable[EntityId]] = None) -> List[SlackTask]:
        """Find all the slack tasks."""
        query_stmt = \
            select(self._slack_task_table) \
                .where(self._slack_task_table.c.slack_task_collection_ref_id == parent_ref_id.as_int())
        if not allow_archived:
            query_stmt = query_stmt.where(self._slack_task_table.c.archived.is_(False))
        if filter_ref_ids:
            query_stmt = query_stmt.where(self._slack_task_table.c.ref_id.in_(fi.as_int() for fi in filter_ref_ids))
    ...
    def remove(self, ref_id: EntityId) -> SlackTask:
        ...
        self._connection.execute(
            delete(self._slack_task_table).where(self._slack_task_table.c.ref_id == ref_id.as_int()))
    ...
    def _row_to_entity(row: Result) -> SlackTask:
        return SlackTask(
            ...
            last_modified_time=Timestamp.from_db(row["last_modified_time"]),
            events=[],
            slack_task_collection_ref_id=EntityId.from_raw(str(row["slack_task_collection_ref_id"])),
            user=SlackUserName.from_raw(row["user"]),
            channel=SlackChannelName.from_raw(row["channel"]) if row["channel"] else None,
            message=row["message"],
            generation_extra_info=PushGenerationExtraInfo.from_db(row["generation_extra_info"]),
