...
from jupiter.domain.big_plans.infra.big_plan_collection_repository import BigPlanCollectionRepository, \
    BigPlanCollectionNotFoundError, BigPlanCollectionAlreadyExistsError
...
class SqliteBigPlanCollectionRepository(BigPlanCollectionRepository):
    ...
    def create(self, entity: BigPlanCollection) -> BigPlanCollection:
        ...
        try:
            ...
            raise BigPlanCollectionAlreadyExistsError(
                f"Big plan collection for workspace {entity.workspace_ref_id} already exists") from err
    ...
    def load_by_parent(self, parent_ref_id: EntityId) -> BigPlanCollection:
        ...
        query_stmt = \
            select(self._big_plan_collection_table)\
                .where(self._big_plan_collection_table.c.workspace_ref_id == parent_ref_id.as_int())
        result = self._connection.execute(query_stmt).first()
        if result is None:
            raise BigPlanCollectionNotFoundError(f"Big plan collection for workspace {parent_ref_id} does not exist")
        return self._row_to_entity(result)
...
class SqliteBigPlanRepository(BigPlanRepository):
    ...
    def save(self, entity: BigPlan) -> BigPlan:
        ...
        result = self._connection.execute(
            ...
            .values(
                ...
                completed_time=entity.completed_time.to_db() if entity.completed_time else None))
        if result.rowcount == 0:
            raise BigPlanNotFoundError(f"Big plan with id {entity.ref_id} does not exist")
        upsert_events(self._connection, self._big_plan_event_table, entity)
        return entity
    ...
    def find_all(
            ...
            filter_ref_ids: Optional[Iterable[EntityId]] = None) -> List[BigPlan]:
    ...
    def find_all_with_filters(
        ...
        query_stmt = \
            ...
            .where(self._big_plan_table.c.big_plan_collection_ref_id == parent_ref_id.as_int())
        if not allow_archived:
            query_stmt = query_stmt.where(self._big_plan_table.c.archived.is_(False))
        if filter_ref_ids:
