...
from jupiter.domain.smart_lists.infra.smart_list_repository import (
    SmartListRepository,
    SmartListAlreadyExistsError,
...
class SqliteSmartListCollectionRepository(SmartListCollectionRepository):
    ...
    def __init__(self, connection: Connection, metadata: MetaData) -> None:
        ...
        self._smart_list_collection_event_table = build_event_table(
            self._smart_list_collection_table, metadata
        )
...
class SqliteSmartListRepository(SmartListRepository):
    ...
    def __init__(self, connection: Connection, metadata: MetaData) -> None:
        ...
        self._smart_list_event_table = build_event_table(
            self._smart_list_table, metadata
        )
    ...
    def create(self, entity: SmartList) -> SmartList:
        ...
        try:
            ...
            raise SmartListAlreadyExistsError(
                f"Smart list with key {entity.key} already exists"
            ) from err
        entity = entity.assign_ref_id(EntityId(str(result.inserted_primary_key[0])))
    ...
    def find_all(
        ...
        filter_keys: Optional[Iterable[SmartListKey]] = None,
    ) -> List[SmartList]:
...
class SqliteSmartListTagRepository(SmartListTagRepository):
    ...
    def __init__(self, connection: Connection, metadata: MetaData) -> None:
        ...
        self._smart_list_tag_event_table = build_event_table(
            self._smart_list_tag_table, metadata
        )
...
class SqliteSmartListItemRepository(SmartListItemRepository):
    ...
    def __init__(self, connection: Connection, metadata: MetaData) -> None:
        ...
        self._smart_list_item_event_table = build_event_table(
            self._smart_list_item_table, metadata
        )
