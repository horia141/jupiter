...
from sqlalchemy import select, MetaData, Table, Column, Integer, Boolean, DateTime, update, insert, \
    Unicode, String, delete, ForeignKey
from sqlalchemy.engine import Result, Connection
from sqlalchemy.exc import IntegrityError

from jupiter.domain.difficulty import Difficulty
from jupiter.domain.eisen import Eisen
from jupiter.domain.persons.infra.person_repository import PersonRepository, PersonAlreadyExistsError, \
    PersonNotFoundError
...
class SqlitePersonRepository(PersonRepository):
    ...
    def __init__(self, connection: Connection, metadata: MetaData) -> None:
        ...
        self._person_table = Table(
            ...
            Column('last_modified_time', DateTime, nullable=False),
            Column('archived_time', DateTime, nullable=True),
            Column('person_collection_ref_id', Integer, ForeignKey("person_collection.ref_id"), nullable=False),
    ...
    def _row_to_entity(row: Result) -> Person:
        return Person(
            ...
            last_modified_time=Timestamp.from_db(row["last_modified_time"]),
            events=[],
            person_collection_ref_id=EntityId.from_raw(str(row["person_collection_ref_id"])),
            ...
            if row["catch_up_period"] is not None else None,
