...
from jupiter.domain.projects.infra.project_repository import (
    ProjectRepository,
...
class SqliteProjectCollectionRepository(ProjectCollectionRepository):
    ...
    def __init__(self, connection: Connection, metadata: MetaData) -> None:
        ...
        self._project_collection_event_table = build_event_table(
            self._project_collection_table, metadata
        )
...
class SqliteProjectRepository(ProjectRepository):
    ...
    def create(self, entity: Project) -> Project:
        ...
        try:
            ...
            raise ProjectAlreadyExistsError(
                f"Project with key {entity.key} already exists"
            ) from err
        entity = entity.assign_ref_id(EntityId(str(result.inserted_primary_key[0])))
    ...
    def load_by_key(
        ...
        query_stmt = (
            ...
            .where(
                self._project_table.c.project_collection_ref_id
                == project_collection_ref_id.as_int()
            )
    ...
    def exchange_keys_for_ref_ids(
        ...
        query_stmt = select(self._project_table.c.ref_id).where(
            self._project_table.c.the_key.in_(project_keys)
        )
