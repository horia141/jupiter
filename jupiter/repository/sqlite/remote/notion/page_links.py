...
from jupiter.remote.notion.infra.page_link_repository import (
    NotionPageLinkRepository,
    NotionPageLinkNotFoundError,
)
...
class SqliteNotionPageLinkRepository(NotionPageLinkRepository):
    ...
    def save(self, notion_page_link: NotionPageLink) -> NotionPageLink:
        ...
        if result.rowcount == 0:
            raise NotionPageLinkNotFoundError(
                f"The Notion page link with key {notion_page_link.key} does not exist"
            )
        return notion_page_link

    def load(self, key: NotionLockKey) -> NotionPageLink:
        """Load a Notion page."""
        query_stmt = select(self._notion_page_link_table).where(
            self._notion_page_link_table.c.the_key == key
        )
        result = self._connection.execute(query_stmt).first()
        if result is None:
            raise NotionPageLinkNotFoundError(
                f"The Notion page link with key {key} does not exist"
            )
    ...
    def remove(self, key: NotionLockKey) -> None:
        ...
        self._connection.execute(
            delete(self._notion_page_link_table).where(
                self._notion_page_link_table.c.the_key == key
            )
        )
