...
from sqlalchemy import insert, MetaData, Table, Column, DateTime, String, update, select, delete, JSON
...
from jupiter.remote.notion.infra.collection_link_repository \
    import NotionCollectionLinkRepository, NotionCollectionLinkNotFoundError
...
class SqliteNotionCollectionLinkRepository(NotionCollectionLinkRepository):
    ...
    def create(self, notion_collection_link: NotionCollectionLink) -> NotionCollectionLink:
        ...
        self._connection.execute(
            insert(self._notion_collection_link_table).values(
                ...
                view_notion_ids={k: str(v) for k, v in notion_collection_link.view_notion_ids.items()},
    ...
    def save(self, notion_collection_link: NotionCollectionLink) -> NotionCollectionLink:
        ...
        result = self._connection.execute(
            ...
            .where(self._notion_collection_link_table.c.the_key == notion_collection_link.key)
            .values(
                page_notion_id=str(notion_collection_link.page_notion_id),
                collection_notion_id=str(notion_collection_link.collection_notion_id),
                view_notion_ids={k: str(v) for k, v in notion_collection_link.view_notion_ids.items()},
    ...
    def load(self, key: NotionLockKey) -> NotionCollectionLink:
        ...
        query_stmt = \
            select(self._notion_collection_link_table)\
            .where(self._notion_collection_link_table.c.the_key == key)
        result = self._connection.execute(query_stmt).first()
        if result is None:
            raise NotionCollectionLinkNotFoundError(f"The Notion collection link with key {key} does not exist")
    ...
    def remove(self, key: NotionLockKey) -> None:
        ...
        self._connection.execute(
            delete(self._notion_collection_link_table)
            .where(self._notion_collection_link_table.c.the_key == key))
    ...
    def _row_to_entity(row: Result) -> NotionCollectionLink:
        return NotionCollectionLink(
            ...
            view_notion_ids={k: NotionId.from_raw(v) for k, v in row["view_notion_ids"].items()},
