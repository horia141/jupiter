...
class NotionChoresManager(ChoreNotionManager):
    ...
    def upsert_trunk(self, parent: NotionWorkspace, trunk: NotionChoreCollection) -> None:
        ...
        self._collections_manager.upsert_collection(
            ...
            view_schemas=[
                ("database_by_project_view_id", NotionChoresManager._DATABASE_BY_PROJECT_VIEW_SCHEMA),
    ...
    def upsert_chores_project_field_options(
        ...
        new_view: JSONDictType = copy.deepcopy(NotionChoresManager._DATABASE_BY_PROJECT_VIEW_SCHEMA)
    ...
    def _copy_row_to_notion_row(
        ...
        if row.ref_id is None:
            raise Exception(f"Chore row '{row.name}' which is synced must have a ref id")
    ...
    def _copy_notion_row_to_row(self, chore_notion_row: CollectionRowBlock) -> NotionChore:
        ...
        return NotionChore(
            ...
            start_at_date=ADate.from_notion(self._global_properties.timezone, chore_notion_row.start_at_date)
            if chore_notion_row.start_at_date else None,
            end_at_date=ADate.from_notion(self._global_properties.timezone, chore_notion_row.end_at_date)
            if chore_notion_row.end_at_date else None,
            last_edited_time=
            Timestamp.from_notion(chore_notion_row.last_edited_time),
            ref_id=EntityId.from_raw(chore_notion_row.ref_id) if chore_notion_row.ref_id else None)
