...
class NotionSmartListItem(
    NotionLeafEntity[
        ...
        "NotionSmartListItem.InverseInfo",
    ]
):
    """A smart list item on Notion-side."""
    ...
    def new_notion_entity(
        entity: SmartListItem, extra_info: DirectInfo
    ) -> "NotionSmartListItem":
        ...
        return NotionSmartListItem(
            ...
            tags=[
                str(extra_info.tags_by_ref_id[t].tag_name) for t in entity.tags_ref_id
            ],
            url=str(entity.url),
        )

    def new_entity(
        self, parent_ref_id: EntityId, extra_info: InverseInfo
    ) -> SmartListItem:
        ...
        return SmartListItem.new_smart_list_item(
            ...
            tags_ref_id=[
                extra_info.tags_by_name[SmartListTagName.from_raw(t)].ref_id
                for t in self.tags
            ],
    ...
    def apply_to_entity(
        self, entity: SmartListItem, extra_info: InverseInfo
    ) -> NotionLeafApplyToEntityResult[SmartListItem]:
        ...
        return NotionLeafApplyToEntityResult.just(
            entity.update(
                ...
                url=UpdateAction.change_to(
                    URL.from_raw(self.url) if self.url else None
                ),
                source=EventSource.NOTION,
