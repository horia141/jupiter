...
class ChoreArchiveService:
    ...
    def do_it(self, chore: Chore) -> None:
        ...
        with self._storage_engine.get_unit_of_work() as uow:
            chore = chore.mark_archived(self._source, self._time_provider.get_current_time())
            uow.chore_repository.save(chore)

            chore_collection = \
                uow.chore_collection_repository.load_by_id(chore.chore_collection_ref_id)

            inbox_task_collection = \
                uow.inbox_task_collection_repository.load_by_workspace(chore_collection.workspace_ref_id)
            inbox_tasks_to_archive = uow.inbox_task_repository.find_all(
                inbox_task_collection_ref_id=inbox_task_collection.ref_id,
                allow_archived=False, filter_chore_ref_ids=[chore.ref_id])
