...
class ProjectLabelUpdateService:
    ...
    def sync(self, workspace: Workspace, projects: Iterable[Project]) -> None:
        ...
        project_labels = [
            NotionFieldLabel(p.notion_link_uuid, p.name, p.created_time)
            for p in projects
        ]
        ...
        self._habit_notion_manager.upsert_habits_project_field_options(
            habit_collection.ref_id, project_labels
        )
        self._chore_notion_manager.upsert_chores_project_field_options(
            chore_collection.ref_id, project_labels
        )
