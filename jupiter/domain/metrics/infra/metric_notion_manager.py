...
class MetricNotionManager(abc.ABC):
    ...
    def upsert_root_page(
        ...
        """Upsert the root page structure for metrics."""
    ...
    def load_all_metric_entries(
            self, metric_collection_ref_id: EntityId, metric_ref_id: EntityId) -> Iterable[NotionMetricEntry]:
        """Load all metric entries."""

    @abc.abstractmethod
    def remove_metric_entry(
            self, metric_collection_ref_id: EntityId, metric_ref_id: EntityId,
            metric_entry_ref_id: Optional[EntityId]) -> None:
        """Remove a metric on Notion-side."""
    ...
    def load_all_saved_metric_entries_ref_ids(
            self, metric_collection_ref_id: EntityId, metric_ref_id: EntityId) -> Iterable[EntityId]:
        """Retrieve all the saved ref ids for the metric entries."""

    @abc.abstractmethod
    def load_all_saved_metric_entries_notion_ids(
            self, metric_collection_ref_id: EntityId, metric_ref_id: EntityId) -> Iterable[NotionId]:
