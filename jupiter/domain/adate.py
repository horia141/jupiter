...
class ADate(Value):
    ...
    def from_raw(timezone: Timezone, datetime_raw: Optional[str]) -> "ADate":
        ...
        try:
            adate = pendulum.parse(
                datetime_raw, tz=pendulum.timezone(str(timezone)), exact=True
            )
    ...
    def from_str(date_raw: str) -> "ADate":
        ...
        return ADate(
            pendulum.parse(date_raw.replace(" 00:00:00", ""), tz=UTC, exact=True), None
        )
    ...
    def to_user_str(timezone: Timezone, adate: Optional["ADate"]) -> str:
        ...
        if adate._the_datetime is not None:
            return cast(
                str,
                adate._the_datetime.in_timezone(
                    pendulum.timezone(str(timezone))
                ).to_datetime_string(),
            )
        else:
