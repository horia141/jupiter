...
class PersonSyncService:
    ...
    def __init__(
            self, storage_engine: DomainStorageEngine, person_person_notion_manager: PersonNotionManager) -> None:
        """Constructor."""
        self._storage_engine = storage_engine
        self._person_notion_manager = person_person_notion_manager
    ...
    def sync(
            ...
            sync_prefer: SyncPrefer) -> Iterable[Person]:
        ...
        if not drop_all_notion_side:
            all_notion_persons = self._person_notion_manager.load_all_persons(person_collection.ref_id)
            all_notion_persons_notion_ids =\
                set(self._person_notion_manager.load_all_saved_person_notion_ids(person_collection.ref_id))
        else:
            self._person_notion_manager.drop_all_persons(person_collection.ref_id)
            all_notion_persons = []
        ...
        for notion_person in all_notion_persons:
            ...
            if notion_person.ref_id is None:
                ...
                self._person_notion_manager.link_local_and_notion_entries(
                ...
                try:
                    self._person_notion_manager.remove_person(
                        person_collection.ref_id, typing.cast(EntityId, notion_person.ref_id))
