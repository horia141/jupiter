...
class SyncUseCase(AppMutationUseCase["SyncUseCase.Args", None]):
    ...
    def _execute(self, context: AppUseCaseContext, args: Args) -> None:
        ...
        if SyncTarget.STRUCTURE in args.sync_targets:
            ...
            notion_inbox_task_collection = NotionInboxTaskCollection.new_notion_entity(
            ...
            notion_smart_list_collection = NotionSmartListCollection.new_notion_entity(
            ...
            notion_slack_task_collection = NotionSlackTaskCollection.new_notion_entity(
        ...
        if SyncTarget.INBOX_TASKS in args.sync_targets:
            all_inbox_tasks = self._sync_inbox_tasks(
                all_big_plans, all_projects, args, workspace
            )
        else:
            with self._storage_engine.get_unit_of_work() as uow:
        ...
        if SyncTarget.BIG_PLANS in sync_targets:
            self._archive_big_plan_inbox_tasks(
                all_inbox_tasks, args, big_plans_by_ref_id, inbox_task_archive_service
            )

        if SyncTarget.HABITS in sync_targets:
            self._archive_habits_inbox_tasks(
                all_inbox_tasks, args, habits_by_ref_id, inbox_task_archive_service
            )

        if SyncTarget.CHORES in sync_targets:
            self._archive_chores_inbox_tasks(
                all_inbox_tasks, args, chores_by_ref_id, inbox_task_archive_service
            )
        ...
        if SyncTarget.METRICS in sync_targets:
            ...
            for metric in all_metrics:
                if (
                    ...
                    LOGGER.info(
                        f"Skipping metric '{metric.name}' on account of filtering"
                    )
                    continue
        ...
        if SyncTarget.PERSONS in sync_targets:
            all_persons_by_ref_id, project = self._sync_persons(
                args, person_collection, projects_by_ref_id, workspace
            )
    ...
    def _sync_slack_tasks_inbox_tasks(
        ...
        for inbox_task in all_slack_inbox_tasks:
            ...
            if (
                ...
                LOGGER.info(
                    f"Skipping inbox task '{inbox_task.name}' on account of inbox task filtering"
                )
                continue
            if not slack_task.last_modified_time.is_within_ten_minutes(
                ...
                LOGGER.info(
                    f"Skipping {inbox_task.name} because it's Slack task was not modified"
                )
                continue
    ...
    def _sync_person_birthday_inbox_tasks(
        ...
        for inbox_task in all_person_birthday_tasks:
            ...
            if (
                ...
                LOGGER.info(
                    f"Skipping inbox task '{inbox_task.name}' on account of inbox task filterring"
                )
                continue
    ...
    def _sync_person_catch_up_inbox_tasks(
        ...
        for inbox_task in all_person_catch_up_tasks:
            ...
            if not person.last_modified_time.is_within_ten_minutes(
                ...
                LOGGER.info(
                    f"Skipping {inbox_task.name} because it's person was not modified"
                )
                continue
            LOGGER.info(f"Syncing inbox task '{inbox_task.name}'")
            if person.archived:
                ...
                schedule = schedules.get_schedule(
                    cast(RecurringTaskGenParams, person.catch_up_params).period,
                    person.name,
                    cast(Timestamp, inbox_task.recurring_gen_right_now),
                    self._global_properties.timezone,
                    None,
    ...
    def _sync_metric_inbox_tasks(
        ...
        for inbox_task in all_metric_collection_tasks:
            ...
            if (
                ...
                LOGGER.info(
                    f"Skipping inbox task '{inbox_task.name}' on account of metric filtering"
                )
                continue
    ...
    def _sync_smart_lists(
        self, all_smart_lists: Iterable[SmartList], args: Args, workspace: Workspace
    ) -> None:
        for smart_list in all_smart_lists:
            if (
                ...
                LOGGER.info(
                    f"Skipping smart list '{smart_list.name}' on account of filtering"
                )
                continue
