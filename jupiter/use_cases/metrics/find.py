...
class MetricFindUseCase(
    AppReadonlyUseCase["MetricFindUseCase.Args", "MetricFindUseCase.Result"]
):
    """The command for finding metrics."""
    ...
    def _execute(
        ...
        with self._storage_engine.get_unit_of_work() as uow:
            ...
            metric_entries = itertools.chain(
                *(
                    uow.metric_entry_repository.find_all(
                        parent_ref_id=m.ref_id, allow_archived=args.allow_archived
                    )
                    for m in metrics
                )
            )
            ...
            collection_project = uow.project_repository.load_by_id(
