...
class ChoreFindUseCase(AppReadonlyUseCase['ChoreFindUseCase.Args', 'ChoreFindUseCase.Result']):
    """The command for finding a chore."""
    ...
    def _execute(self, context: AppUseCaseContext, args: Args) -> 'Result':
        ...
        with self._storage_engine.get_unit_of_work() as uow:
            ...
            inbox_tasks = uow.inbox_task_repository.find_all_with_filters(
                ...
                allow_archived=True, filter_chore_ref_ids=(bp.ref_id for bp in chores))

        return ChoreFindUseCase.Result(
            chores=[
                ChoreFindUseCase.ResultEntry(
                    chore=rt,
                    inbox_tasks=[it for it in inbox_tasks if it.chore_ref_id == rt.ref_id])
                for rt in chores])
