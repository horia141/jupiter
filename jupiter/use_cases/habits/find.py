...
class HabitFindUseCase(AppReadonlyUseCase['HabitFindUseCase.Args', 'HabitFindUseCase.Result']):
    """The command for finding a habit."""
        ...
        return HabitFindUseCase.Result(
            habits=[
                HabitFindUseCase.ResultEntry(
                    habit=rt,
                    inbox_tasks=[it for it in inbox_tasks if it.habit_ref_id == rt.ref_id])
                for rt in habits])
