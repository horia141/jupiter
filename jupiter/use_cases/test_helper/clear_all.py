...
class ClearAllUseCase(AppMutationUseCase["ClearAllUseCase.Args", None]):
    ...
    def _execute(self, context: AppUseCaseContext, args: Args) -> None:
        ...
        with self._storage_engine.get_unit_of_work() as uow:
            LOGGER.info("Clearing vacations")
        ...
        for vacation in all_vacations:
            LOGGER.info(f"Removing vacation '{vacation.name}'")
            vacation_remove_service.do_it(vacation)
        ...
        for habit in all_habits:
            LOGGER.info(f"Removing habit '{habit.name}'")
            habit_remove_service.remove(habit.ref_id)

        with self._storage_engine.get_unit_of_work() as uow:
            LOGGER.info("Clearing chores")
        ...
        for chore in all_chores:
            LOGGER.info(f"Removing chore '{chore.name}'")
            chore_remove_service.remove(chore.ref_id)

        with self._storage_engine.get_unit_of_work() as uow:
            LOGGER.info("Clearing big plans")
        ...
        with self._storage_engine.get_unit_of_work() as uow:
            LOGGER.info("Clearing smart lists")
        ...
        for smart_list in all_smart_lists:
            LOGGER.info(f"Removing smart list '{smart_list.name}'")
        ...
        for metric in all_metrics:
            LOGGER.info(f"Removing metric '{metric.name}'")
            metric_remove_service.execute(metric_collection, metric)

        with self._storage_engine.get_unit_of_work() as uow:
            LOGGER.info("Clearing persons")
        ...
        for person in all_persons:
            LOGGER.info(f"Removing person '{person.name}'")
            person_remove_service.do_it(person_collection, person)

        with self._storage_engine.get_unit_of_work() as uow:
            LOGGER.info("Clearing Slack tasks")
        ...
        for slack_task in all_slack_tasks:
            LOGGER.info(f"Removing slack task '{slack_task.simple_name}'")
            slack_task_remove_service.do_it(slack_task)

        with self._storage_engine.get_unit_of_work() as uow:
            LOGGER.info("Clearing inbox tasks")
        ...
        with self._use_case_storage_engine.get_unit_of_work() as uow:
            LOGGER.info("Clearing use case invocation records")
