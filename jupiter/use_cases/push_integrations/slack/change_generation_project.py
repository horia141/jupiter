...
class SlackTaskChangeGenerationProjectUseCase(AppMutationUseCase['SlackTaskChangeGenerationProjectUseCase.Args', None]):
    ...
    def _execute(self, context: AppUseCaseContext, args: Args) -> None:
        ...
        with self._storage_engine.get_unit_of_work() as uow:
            ...
            if args.generation_project_key is not None:
                generation_project = \
                    uow.project_repository.load_by_key(project_collection.ref_id, args.generation_project_key)
                generation_project_ref_id = generation_project.ref_id
            else:
                generation_project = uow.project_repository.load_by_id(workspace.default_project_ref_id)
            ...
            slack_task_collection =\
                slack_task_collection.change_generation_project(
                    generation_project_ref_id=generation_project_ref_id, source=EventSource.CLI,
                    modified_time=self._time_provider.get_current_time())

            uow.slack_task_collection_repository.save(slack_task_collection)

            slack_tasks =\
                uow.slack_task_repository.find_all(parent_ref_id=slack_task_collection.ref_id, allow_archived=False)
            slack_tasks_by_ref_id = {st.ref_id: st for st in slack_tasks}

        if old_generation_project_ref_id != generation_project_ref_id and len(slack_tasks) > 0:
            with self._storage_engine.get_unit_of_work() as inbox_task_uow:
                inbox_task_collection = inbox_task_uow.inbox_task_collection_repository.load_by_parent(workspace.ref_id)
                ...
                for inbox_task in all_generated_inbox_tasks:
                    slack_task = slack_tasks_by_ref_id[cast(EntityId, inbox_task.slack_task_ref_id)]
                    update_inbox_task = \
                        inbox_task.update_link_to_slack_task(
            ...
            for inbox_task in updated_generated_inbox_tasks:
                direct_info = \
                    NotionInboxTask.DirectInfo(
                        all_projects_map={generation_project.ref_id: generation_project}, all_big_plans_map={})
                notion_inbox_task = \
