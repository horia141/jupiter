...
class SlackTaskUpdateUseCase(AppMutationUseCase['SlackTaskUpdateUseCase.Args', None]):
    ...
    def _execute(self, context: AppUseCaseContext, args: Args) -> None:
        ...
        with self._storage_engine.get_unit_of_work() as uow:
            ...
            if args.generation_name.should_change or \
                args.generation_status.should_change or \
                args.generation_eisen.should_change or \
                args.generation_difficulty.should_change or \
                args.generation_actionable_date.should_change or \
                args.generation_due_datee.should_change:
                generation_extra_info=UpdateAction.change_to(
                    PushGenerationExtraInfo(
                        timezone=workspace.timezone,
                        name=args.generation_name.or_else(slack_task.generation_extra_info.name),
                        status=args.generation_status.or_else(slack_task.generation_extra_info.status),
                        eisen=args.generation_eisen.or_else(slack_task.generation_extra_info.eisen),
                        difficulty=args.generation_difficulty.or_else(slack_task.generation_extra_info.difficulty),
                        actionable_date=args.generation_actionable_date.or_else(
                            slack_task.generation_extra_info.actionable_date),
                        due_date=args.generation_due_datee.or_else(slack_task.generation_extra_info.due_date)))
            else:
            ...
            generated_inbox_task = \
                uow.inbox_task_repository.find_all_with_filters(
                    ...
                    filter_slack_task_ref_ids=[slack_task.ref_id])[0]

            generated_inbox_task = \
                generated_inbox_task.update_link_to_slack_task(
            ...
            project = uow.project_repository.load_by_id(generated_inbox_task.project_ref_id)

        notion_slack_task = \
            self._slack_task_notion_manager.load_leaf(slack_task.slack_task_collection_ref_id, slack_task.ref_id)
        notion_slack_task = notion_slack_task.join_with_entity(slack_task, None)
        self._slack_task_notion_manager.save_leaf(slack_task.slack_task_collection_ref_id, notion_slack_task)
        ...
        notion_generated_inbox_task = \
            self._inbox_task_notion_manager.load_leaf(
                generated_inbox_task.inbox_task_collection_ref_id, generated_inbox_task.ref_id)
        notion_generated_inbox_task = \
            notion_generated_inbox_task.join_with_entity(generated_inbox_task, inbox_task_direct_info)
        self._inbox_task_notion_manager.save_leaf(
            generated_inbox_task.inbox_task_collection_ref_id, notion_generated_inbox_task)
